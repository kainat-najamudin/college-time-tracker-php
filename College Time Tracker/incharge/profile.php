...
$user = fetch_all("SELECT * FROM academicsincharge WHERE InchargeID=$uid")[0] ?? [];
$success = $error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($email)) {
        ...
        $dept_s  = mysqli_real_escape_string($conn, $dept);
        if (mysqli_query($conn, "UPDATE academicsincharge SET Name='$name_s', Email='$email_s', Department='$dept_s' WHERE InchargeID=$uid")) {
...
<div class="row g-4">
    <div class="col-lg-4">
        <div class="data-card text-center">
            ...
            <span style="background:#eff6ff;color:#2563eb;padding:5px 16px;border-radius:50px;font-size:0.82rem;font-weight:700;">
                <i class="fa-solid fa-user-tie me-1"></i>Academic In-Charge
            </span>
