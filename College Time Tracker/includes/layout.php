...
function dashboard_start($title, $subtitle = '') {
    ...
    echo '<a class="sidebar-brand" href="' . app_url(role_dashboard_path($role)) . '"><i class="fa-solid fa-clock"></i><span>CTT</span></a>';
    echo '<div class="sidebar-role">' . h(ucfirst($role)) . ' Panel</div>';
    echo '<nav class="sidebar-nav">';
    foreach (sidebar_links($role) as $link) {
        echo '<a href="' . app_url($link[1]) . '"><i class="fa-solid ' . h($link[2]) . '"></i><span>' . h($link[0]) . '</span></a>';
    }
    echo '</nav>';
    echo '<a class="logout-link" href="' . app_url('logout.php') . '"><i class="fa-solid fa-right-from-bracket"></i> Logout</a>';
    echo '</aside>';
    ...
    echo '<header class="dashboard-topbar">';

    /* Hamburger — mobile only */
echo '<button class="hamburger-btn d-lg-none" onclick="toggleSidebar()" aria-label="Toggle menu" style="background:transparent;border:none;cursor:pointer;padding:6px 8px;display:flex;flex-direction:column;gap:6px;align-items:center;justify-content:center;"><span style="display:block;width:20px;height:2px;background:#000;border-radius:2px;"></span><span style="display:block;width:20px;height:2px;background:#000;border-radius:2px;"></span><span style="display:block;width:20px;height:2px;background:#000;border-radius:2px;"></span></button>';
    /* Title */
    echo '<div><h1>' . h($title) . '</h1><p>' . h($subtitle) . '</p></div>';

    /* Right side: clock + user pill */
    ...
    echo '</header>';
    echo '<section class="dashboard-content">';
    render_flash();
}
